...
TowerRemote::TowerRemote(int r, int c, vector<Enemy *>& enemy_all) : Tower(r, c, enemy_all)
{
    type = 5; //普通远程塔
    step = 0;
    all_health = 100;
...
//攻击enemy2attack里的敌人
void TowerRemote::attack_enemys(vector<Enemy *>& enemy2attack) {
    ...
    if(counter >= interval) { //间隔过后才发射子弹
        ...
        switch (step) {
        case 0: {
            picture = "../source/TBottle-shoot1.png";
            step = 1;
            break;
        }
        case 1: {
            picture = "../source/TBottle-shoot2.png";
            step = 0;
        }
        }
...
int TowerRemote::update_each() {
    ...
    for(auto& enemy : enemy_all) {
        ...
        int e_cx =  enemy->x + enemy->weight/2, e_cy =  enemy->y + enemy->height/2;
        //攻击范围内的地面敌人
        if(enemy->type <= 4  && Distance(t_cx, t_cy, e_cx, e_cy) < this->range) {
